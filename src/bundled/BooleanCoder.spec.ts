...
describe('BooleanCoder', () => {
  it('can be initialized', () => {
    BooleanCoder()
    BooleanCoder({})
    BooleanCoder({ coerceOnDecode: true })
  })
  ...
  it('coerces type on decode()', () => {
    const coder = BooleanCoder({ coerceOnDecode: true })

    expect(coder.decode(true)).to.eq(true)
    expect(coder.decode(false)).to.eq(false)
    expect(coder.decode(0 as any)).to.eq(false)
    expect(coder.decode('0' as any)).to.eq(false)
    expect(coder.decode('false' as any)).to.eq(false)
    expect(coder.decode('FALSE' as any)).to.eq(false)
    expect(coder.decode(1 as any)).to.eq(true)
    expect(coder.decode('1' as any)).to.eq(true)
    expect(coder.decode('true' as any)).to.eq(true)
    expect(coder.decode('TRUE' as any)).to.eq(true)
    expect(() => coder.decode(undefined as any)).to.throw(DecodingError)
    expect(() => coder.decode(null as any)).to.throw(DecodingError)
    expect(() => coder.decode(-1 as any)).to.throw(DecodingError)
